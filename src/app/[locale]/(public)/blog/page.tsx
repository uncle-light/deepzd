    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <div className="pt-32 pb-24 px-4 md:px-6">
        <div className="max-w-3xl mx-auto">
          ...
          {allTags.length > 0 && (
            <TagFilter tags={tagsForFilter} locale={locale} allLabel={t("allTags")} />
          )}
                ...
                <article key={post.slug} className="group">
                  <Link href={`/${locale}/blog/${post.slug}`}>
                    <div className="flex items-center gap-3 text-sm text-[var(--gray-500)] mb-3">
                      ...
                      {post.read_time && <span>{post.read_time} {t("minRead")}</span>}
                    </div>
                    <h2 className="text-xl md:text-2xl font-semibold mb-3 text-[var(--foreground)] group-hover:text-[var(--accent)] transition-colors">
                      {title}
                    </h2>
                    {excerpt && (
                      <p className="text-[var(--gray-400)] leading-relaxed mb-4">{excerpt}</p>
                    )}
                    {post.tags && post.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {post.tags.map((tag) => (
                          <span key={tag.slug} className="px-2 py-1 text-xs rounded border border-[var(--border)] text-[var(--gray-500)]">
          ...
          {filteredPosts.length === 0 && (
            <p className="text-center text-[var(--gray-500)] py-12">{t("noPosts")}</p>
          )}
        </div>
      </div>
    </div>
